...
# Fullständig, körbar kod. Klistra in och kör i en cell.
#
# Funktion:
# - Skalar upp halva–halva-relationen $r=1.5$ från 1 spole → 3 → 9 → 27 (3×3).
# - För varje $m \in \{1,3,9,27\}$ beräknas kalibrerad effekt $P^*$ över $\theta$ för skalfamiljen:
#     $f_{i,k}=\theta\cdot r^{\,k+\frac{i}{m}}$, $i=0,\dots,m-1$, $k\in[k_{\min},k_{\max}]$.
# - Jämför $r=1.5$ mot alternativ (altA = $\varphi$ och valbart altB = $\sqrt{2}$) med samma $m$-struktur.
# - Producerar CSV/JSON med resultat och skriver en tydlig konsol-rapport (ingen PDF/HTML krävs).
#
# Nyckelmått:
# - $U=\frac{P^*_{1.5}}{P^*_{alt}}-1$, $D=P^*_{1.5}-P^*_{alt}$.
# - Statistik per m: $\bar U$, $SE$, $95\%$-CI, ensidigt $t/p$, vinstfrekvens $\hat p$ + Wilson-CI, permutation $p_{perm}$.

import sys, subprocess, os, math, json, time, glob
from pathlib import Path

# === (Valfritt) minimal paketsetup — Colab har oftast allt redan ===
def _pip_try(pkgs):
    try:
...
_pip_try(["numpy", "pandas"])  # snabba basberoenden
...
# ------------- Syntetiska spektra (bara om ./spectra saknar CSV) -------------
...
def theta_grid_from_range_multi(fmin, fmax, r, kmin, kmax, m, n_theta_base):
    # Fraktionsförskjutningar $\frac{i}{m}$ ger skalfaktorer $g_i=r^{i/m}$; spridning upp till $r^{(m-1)/m}$.
    ...
    # Tätare $\theta$ för större m (enkel heuristik):
...
def captured_power_multi(theta, r, kmin, kmax, f, s, m):
    # $P(\theta;H_{r,m},S)=\sum_{i=0}^{m-1}\sum_{k=k_{\min}}^{k_{\max}} S(\theta\cdot r^{\,k+i/m})$.
...
def run_rp9_multiplex(
    ...
    n_theta_base=200,
    m_list=(1,3,9,27),
    altA="phi",                  # 'phi', 'sqrt2' eller numeriskt (t.ex. '1.6')
    altB=True,                   # True => jämför dessutom mot $r=\sqrt{2}$ i ett separat block
    permutations=5000,
    seed=123
):
    r_main = 1.5
    ...
    # Kör block B (√2) om begärt och ej samma som A
    tagB = None
...
# === Kör nu: m = 1,3,9,27; altA=φ; altB=√2 ===
result = run_rp9_multiplex(
    spectra_dir="./spectra",   # lägg egna CSV här; annars genereras syntetiska så att körningen ALLTID fungerar
    kmin=-8, kmax=8,
    n_theta_base=200,
    m_list=(1,3,9,27),
    altA="phi",
